...
const state = {
  count: 0,
  items1: [{ title: 'foo', count: 0, key: '0' }, { title: 'foo', count: 0, key: '1' }],
  items2: [
...
const actions = {
  moveItemByIndex: ({ source_module, oldIndex, target_module, newIndex }) => state => {
    var source_item = state[source_module][oldIndex]
    var newSourceItems = state[source_module].filter((item, index) => {
      return oldIndex !== index
    })

    var items = source_module === target_module ? newSourceItems : state[target_module]
  ...
  add_count: ({ module, key }) => state => ({
    [module]: state[module].map(
      (item, idx) => (item.key === key ? Object.assign({}, item, { count: item.count + 1 }) : item)
    ),
  }),
}
...
var counter = 0
const ListItem = showHandle => props => {
  let { title, key, count } = props.data
  return (
    <div class={'card ' + props.class} data-key={key} key={key}>
      {showHandle ? <span class="drag-handle">☰</span> : null}
      {title} key: {key}, count: {count}{' '}
      <button onclick={() => props.onadd({ module: props.module, key: key })}>+</button> {counter++}
    </div>
  )
}

const ListItem1 = ListItem(true)
const ListItem2 = ListItem(false)
    ...
    <div oncreate={init}>
      ...
      >
        {state.items1.map(item => (
          <ListItem1 class="card1" data={item} module="items1" onadd={actions.add_count} />
        ))}
      </DragableList>
      List 2
      <DragableList
      ...
      >
        {state.items2.map(item => (
          <ListItem2 class="card2" data={item} module="items2" onadd={actions.add_count} />
        ))}
      </DragableList>
      Debug
      <div>
